...
const Form = {
    ...
    init () {
        ...
        .then(json => Form.el.append(...Object.entries(json).map(([name, values]) => 
            E(`fieldset#${name}>div`, E.checkboxes(Array.isArray(values) ? 
                values.map(c => ({name, value: c, label: `${c}`}) ) : 
                Object.entries(values).map(([c, label]) => ({name, value: c, label}) ))
            ))
        ));
        Form.el.Q('#char div').append(...E.checkboxes([...Array(20)].map((_, i) => ({
            label: {classList: 'char', '--x': i % 8, '--y': Math.floor(i / 8)}, 
            name: 'char', value: i,
        }) )));
    },
    events () {
...
const Image = {
    ...
    download: figure => {
        ...
        E('img', {
            src: E(figure).get('--sprite').match(/https.+?png/)[0],
            onload: ev => {
                canvas.getContext('2d').drawImage(ev.target, 
                    figure.id % 10 * 128, Math.floor(figure.id / 10) % 10 * 128, 
                    128, 128, 0, 0, 128, 128
                );
                E('a', {
...
const Query = href =>
    ...
    .then(re => {
        ...
        Q('#result')[href ? 'replaceChildren' : 'append'](...re.map(({ ID, char, name, desc }) => E(`gc-item#${ID}`, {
            title: name,
            dataset: {char},
